...
class LegacyWebSocketManager:
    def __init__(self):
        ...
        self.message_queue = queue.Queue()  # Use regular queue for sync operations
        self._loop = None  # Add loop reference
        self._broadcast_executor = ThreadPoolExecutor(max_workers=4)
        # asyncio.create_task(self.broadcast_worker())

    async def start(self):
        """Call this after event loop is running"""
    ...
    def sync_send_message(self, websocket: WebSocket, message: Dict[str, Any]):
        """
        Synchronous message sender with error handling
        """
    ...
    def sync_broadcast_tensor_update(self, tensor_name: str, tensor_data: Any):
        """
        Thread-safe tensor update broadcast
        """
        def _run_in_executor():
            compressed = self._compress_tensor_data(tensor_data)
            self.sync_broadcast({
                "type": "tensor_update",
                "tensor_name": tensor_name,
                "data": compressed
        ...
        self._broadcast_executor.submit(_run_in_executor)

    # Helper methods
    def _compress_tensor_data(self, data: Any) -> bytes:
        """Mock compression method - implement actual compression"""
        return str(data).encode()

    def set_event_loop(self, loop: asyncio.AbstractEventLoop):
        """
        Allow explicit event loop setting from async context
        """
        self._loop = loop
    ...
    def shutdown(self):
        ...
        Cleanup resources
        """
        self._broadcast_executor.shutdown(wait=True)
